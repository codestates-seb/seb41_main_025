...
const Tving = () => {
  ...
  const [borards] = useFetch(`http://whatu1.kro.kr:8080/boards/tving?page=1&size=10`, request)
  ...
  const submitcommit = async (e) => {
    ...
    await axios.post(`http://whatu1.kro.kr:8080/boards/tving`,bodyJSON,{
      headers: {
        "Content-Type":'application/json',
        "Authorization": localStorage.getItem("accessToken")
      }
  ...
  const deleteBoard = async (tvingBoardId) => {
    ...
    await
    axios.delete (`http://whatu1.kro.kr:8080/boards/tving/${tvingBoardId}`,{
      headers: {
        "Content-Type":'application/json',
        "Authorization": localStorage.getItem("accessToken")
      }
    ...
    return (
      ...
      (
        ...
        <div className="userInfo">
          <button>수정</button>
          <button onClick={() => {deleteBoard(item.tvingBoardId)}}>삭제</button>
